...
const ProductList = () => {
  ...
  const fetchSellerProduct = async () => {
    const { data } = await axiosConfig.get("/api/orders/all?status=Accepted");
      ...
      {loading ? (
        <Loading />
                ...
                <div className="flex flex-col items-center justify-center h-full">
                  <Image
                    src={assets.box_icon} // Ensure `empty_box` exists in your assets
                    alt="No Orders"
                    width={150}
                    height={150}
                  />
        ...
        <div className="w-full md:p-10 p-4">
          <h2 className="pb-4 text-lg font-medium">All Accepted Orders</h2>
              ...
              <tbody className="text-sm text-gray-500">
                ...
                {products.length === 0 && (
                  <tr className="border-t border-gray-500/20 flex items-center justify-start">
                    <td className="w-full  p-5"> ooops! no Accepted orders</td>
                  </tr>
                )}
              </tbody>
            </table>
